...
@Component({
    tag: 'agc-calving-season-inputs'
})
export class AgcCalvingSeasonInputs {
    @Prop() socket: string = ""
    @State() data: any
            ...
            <section data-wizard-results ref={c => this.section = c as HTMLElement}>
                ...
                <div style={{display: this.ready ? 'block' : 'none'}}>
                    {this.data && (<ul class="agc-results">
                            <li>
                                <h2 data-i18n="results.bull-turn-out-date">Bull Turn Out Date</h2>
                                <span class="agc-results__value">{formatDate(this.data['bullTurnOutDate'])}</span>
                            </li>
                            <li>
                                <h2 data-i18n="results.breeding-season-length">Breeding Season Length</h2>
                                <span class="agc-results__value">{this.data['breedingSeasonLength']}</span>
                                <sub>Days</sub>
                            </li>                     
                        </ul>)}
                </div>
